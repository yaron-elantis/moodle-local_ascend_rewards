...
/**
 * Cache warming helper - Pre-loads cache for badges user is close to earning
 */
class cache_warmer {
    ...
    public static function warm_user_cache($userid, $courseid = null) {
        ...
        foreach ($courses as $cid) {
            ...
            foreach ($close_badges as $badgeid) {
                ...
                if (!$DB->record_exists('local_ascend_badge_cache', [
                    'userid' => $userid,
                    'courseid' => $cid,
                    'badgeid' => $badgeid
                ])) {
        ...
        foreach ($missing_badges as $badgeid) {
            switch ($badgeid) {
                case 6: // Getting Started - 1 completion
                    if ($completion_count >= 1) $close[] = $badgeid;
                    break;
                case 4: // On a Roll - 3 completions
                    if ($completion_count >= 2) $close[] = $badgeid;
                    break;
                case 5: // Halfway Hero - 50%
                    if ($completion_pct >= 40) $close[] = $badgeid;
                    break;
                case 7: // Mission Complete - 100%
                    if ($completion_pct >= 90) $close[] = $badgeid;
                    break;
